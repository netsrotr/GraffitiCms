...
public partial class graffiti_admin_marketplace_Catalog : AdminControlPanelPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                var marketplaceBaseUrl = Marketplace.BaseUrl;
                string messageText = "An unexpected error has occurred connecting to the marketplace. The <a href=\"{0}/\" target=\"_blank\">Graffiti Marketplace</a> is where you can find new themes, widgets, and plugins. Please try again later.";
                if (ex is System.Security.SecurityException)
                    messageText = "Your security settings do not allow you to access the marketplace from within the Control Panel. To find new themes, widgets, and plugins, please visit the <a href=\"{0}/\" target=\"_blank\">Graffiti Marketplace</a>.";

                Message.Type = StatusType.Error;
                Message.Text = string.Format(messageText, marketplaceBaseUrl);
